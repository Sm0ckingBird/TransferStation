...
static inline int ring_buffer_idx_inc(int x) { return ++x == PSTRACE_BUF_SIZE ? 0 : x; }

/* Assume pstrace_lock is held */
static int pstrace_get_buf(struct pstrace_kernel *pst, struct pstrace *buf, int limit)
{
	...
	do {
		memcpy(&buf[nr_entries], &pst->entry[head], sizeof(struct pstrace));
		nr_entries++;
		limit--;
...
static void copy_to_blocked_gets(bool unconditional)
{
	...
	list_for_each_safe(evt_list, tmp, &pstrace_evt_list) {
		struct pstrace_evt *evt = list_entry(evt_list, struct pstrace_evt, head);

		pst_counter = atomic_read(&pst->counter);
		if (!evt->copied && (unconditional || evt->counter <= pst_counter)) {
			int limit = evt->counter - pst_counter + PSTRACE_BUF_SIZE;
...
void pstrace_add(struct task_struct *p, long state)
{
	...
	if (state & PSTRACE_ADD_RUNNABLE)
		pst->entry[tail].state |= state ? TASK_RUNNABLE : 0; // pick up TASK_RUNNABLE
	strncpy(pst->entry[tail].comm, p->comm, sizeof(p->comm));
	pst->entry[tail].pid = p->tgid;
	pst->entry[tail].tid = p->pid;
...
{
	if (pid != -1 && pid != 0 && !pid_task(find_vpid(pid), PIDTYPE_PID))
		return -ESRCH;
